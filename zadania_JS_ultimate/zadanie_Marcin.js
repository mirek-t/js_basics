...
const meatEaters = animals
  .flatMap((animal) => Object.values(animal)[0])
  .filter(
    (animal) =>
      animal.habitat.includes("Grassland") && animal.diet.includes("Carnivore")
  )
  .map((animal) => animal.name);

const getPlaces = animals
  .flatMap((animal) => Object.values(animal)[0])
  .filter((animal) => animal.weightInKg > 1 && animal.lifespanInYears >= 10)
  .map((animal) => animal.name);

const biggerThan50 = animals
  .flatMap((animal) => Object.values(animal)[0])
  .filter((animal) => animal.diet.includes("Omnivore") && animal.sizeInCm > 50)
  .map((animal) => animal.name);
